...
def process_ocr_result_cpu(ocr_result):
    ...
    text_lines, confidences, coordinates_list = [], [], []
...
def health():
    ...
    uptime = time.time() - server_stats['startup_time']
    return jsonify({
        'status': 'ok',  # <-- CLAVE para que el healthcheck del contenedor pase
        'ocr_ready': ocr_initialized,
...
def analyze_file_ultra():
    ...
    try:
        ...
        ultra_output.append("=" * 60)
        ultra_output.append(
            f"📊 Orientaciones: {orientations.get('horizontal', 0)} horiz, "
            f"{orientations.get('vertical', 0)} vert, {orientations.get('rotated', 0)} rotadas"
        )
...
# Nota: en Docker usamos waitress-serve desde el Dockerfile (puerto 8000),
# pero dejamos este bloque para ejecución local.
if __name__ == '__main__':
    logger.info("💻 OCR Server CPU Optimizado v3.0 iniciando...")
    if initialize_ocr_cpu():
        logger.info("✅ Modelos OCR CPU pre-cargados")
    from waitress import serve
    logger.info("🚀 Usando Waitress (prod) en 0.0.0.0:8000")
